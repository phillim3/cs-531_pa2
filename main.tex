...
Examining the performance of the puzzle solver using our three different heuristics, we find distinct differences between the heuristics. 
...
\begin{figure}[H]%
	\centering\begin{tabular}{c}
		\includegraphics[scale=0.4]{plots/scramble-algorithm-avgmoves.png}\\
		Scramble Size vs. Algorithm , Average Moves \\[1ex]% add a little extra space
		\includegraphics[scale=0.4]{plots/scramble-heuristic-avgmoves.png}\\
		Scramble Size vs. Heuristic , Average Moves\\[1ex]
	\end{tabular}
	\caption{Performance of Algorithm vs. Scramble Size}%
	\label{fig:summary_avgmoves}%
\end{figure}
...
\begin{figure}[H]%
	\centering\begin{tabular}{c}
		...
		Scramble Size vs. Algorithm , Average Nodes \\[1ex]% add a little extra space
		\includegraphics[scale=0.4]{plots/scramble-heuristic-avgnodes.png}\\
		Scramble Size vs. Heuristic , Average Nodes \\[1ex]
	\end{tabular}
	\caption{Performance of Algorithm vs. Scramble Size}%
	\label{fig:summary_avgnodes}%
\end{figure}
...
\begin{figure}[H]%
	\centering\begin{tabular}{c}
		...
		Scramble Size vs. Algorithm , Average Computation Time (microsec) \\[1ex]% add a little extra space
		\includegraphics[scale=0.4]{plots/scramble-heuristic-avgcomp.png}\\
		Scramble Size vs. Heuristic , Average Computation Time (microsec)\\[1ex]
	\end{tabular}
	\caption{Performance of Algorithm vs. Scramble Size}%
	\label{fig:summary_avgmoves}%
\end{figure}
...
We found from running 10,000 trials for each of the scramble sizes, heuristics, and algorithms, we found that the IDA\textsuperscript{*} algorithm requires about 10-15\% more computation time (the included data reflects a 11.3\% increase). Of course, IDA\textsuperscript{*} ends up expanding many more nodes than RBFS due to its iterative nature which ends up repeating work that was done in previous iterations.

\section{Discussion}
Learning different heuristics were very interesting. The simple Manhattan distance heuristic performed the best in every class, trumping the more complex or clever heuristics.

We expected to see cycles in the solutions, where common states surfaced. However this intuition did not pan out.

Most perplexing, certain board states need an enormous amount of node expansion to find a solution. These board configurations seem common enough, but whatever shared characteristics these board states have, they are not readily apparent.
