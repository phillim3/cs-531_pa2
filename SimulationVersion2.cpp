...
void Critic::rate_performance(const Environment &env)
{
    ...
    clean_percent = 100. * clean / total;
    if(clean_percent==75 && bleh==true)
    {
        bleh=false;
        cout << step_counter << "," << clean_percent << "," ;
    }
...
int main()
{
    ...
    for(int i = 0; i < 50; i++)
    {
        env.execute(stochastic1, critic, false);
        cout << step_counter << "," << clean_percent << "," <<"trial_"<<i<<endl;
        bleh=true;
    }
    return 0;
}